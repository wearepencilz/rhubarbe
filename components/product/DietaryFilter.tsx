...
export default function DietaryFilter({ 
  ...
  const handleToggle = (flag: DietaryFlag) => {
    ...
    if (newSelected.has(flag)) {
      newSelected.delete(flag);
    } else {
      newSelected.add(flag);
    }
  ...
  return (
    <div className={`bg-white rounded-lg border border-gray-200 p-4 ${className}`}>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-gray-900">Dietary Preferences</h3>
      ...
      {selectedFlags.size > 0 && (
        <div className="mt-3 pt-3 border-t border-gray-200">
          <p className="text-xs text-gray-600">
            Showing products matching {selectedFlags.size} filter{selectedFlags.size > 1 ? 's' : ''}
          </p>
        </div>
      )}
    </div>
