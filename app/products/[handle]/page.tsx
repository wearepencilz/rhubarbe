...
function ShopifyProductView({ product }: { product: any }) {
  return (
    <>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
              <div>
                <h1 className="text-3xl md:text-4xl uppercase tracking-widest mb-3" style={{ fontFamily: 'var(--font-neue-montreal)', fontWeight: 500 }}>{product.title}</h1>
              ...
              <AvailabilityBadge availability={product.availability} preorderDate={product.preorderDate} preorderDisclaimer={product.preorderDisclaimer} />
              ...
              {product.options.length > 0 && product.options[0].name !== 'Title' && (
                <div className="space-y-4">
                  {product.options.map((option: any) => (
                    <div key={option.name}>
                      <label className="block text-xs uppercase tracking-widest mb-2" style={{ fontFamily: 'var(--font-diatype-mono)' }}>{option.name}</label>
                      <div className="flex flex-wrap gap-2">
                        {option.values.map((value: string) => (
                          <button key={value} className="px-4 py-2 border rounded hover:border-black transition-colors text-sm">{value}</button>
                        ))}
                      </div>
                    </div>
                  ))}
      ...
      <main className="min-h-screen pt-32 pb-16 px-4 md:px-8">
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
              <div>
                <h1 className="text-3xl md:text-4xl uppercase tracking-widest mb-3" style={{ fontFamily: 'var(--font-neue-montreal)', fontWeight: 500 }}>{displayName}</h1>
                {product.price != null && product.price > 0 && (
                  <span className="text-2xl" style={{ fontFamily: 'var(--font-diatype-mono)' }}>${(product.price / 100).toFixed(2)}</span>
                )}
              </div>
...
function AvailabilityBadge({ availability, preorderDate, preorderDisclaimer }: { availability: string; preorderDate?: string; preorderDisclaimer?: string }) {
