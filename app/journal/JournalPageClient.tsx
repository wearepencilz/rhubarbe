...
export default function JournalPageClient({ entries }: { entries: any[] }) {
  const { T, locale } = useT();
  ...
  return (
    <main className="bg-white min-h-screen">
      {hero ? (
        <Link href={`/journal/${hero.slug}`} className="block group relative w-full overflow-hidden" style={{ height: 'clamp(480px, 75vh, 860px)' }}>
          {hero.coverImage ? (
            <img
              src={hero.coverImage}
...
function StoryCard({ story, featured, locale, T }: { story: any; featured?: boolean; locale: string; T: any }) {
  ...
  return (
    <Link href={`/journal/${story.slug}`} className={`group block ${featured ? 'md:col-span-2' : ''}`}>
      {story.coverImage && (
