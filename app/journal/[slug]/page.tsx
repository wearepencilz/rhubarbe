...
async function getStory(slug: string) {
  try {
    return await journalQuery.getByIdOrSlug(slug);
  } catch {
    return null;
  }
}

async function getRelatedEntries(currentId: string, tags: string[]) {
  try {
    const all = (await journalQuery.list()) as any[];
    return all
...
export default async function JournalEntryPage({ params }: { params: { slug: string } }) {
  ...
  const related = await getRelatedEntries(story.id, tags);
    ...
    <main className="bg-white min-h-screen">
      ...
      <div className="px-4 md:px-8 max-w-[672px] mx-auto pt-14 pb-24">
        ...
        <div className="mt-10 pt-8 border-t border-[#333112]/10 flex items-center gap-6">
          ...
          <a
            href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent(`${process.env.NEXT_PUBLIC_BASE_URL || ''}/journal/${story.slug}`)}`}
            target="_blank"
      ...
      {related.length > 0 && (
        <section className="border-t border-[#333112]/10 px-4 md:px-8 py-16 md:py-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-12">
            {related.map((s: any) => (
              <Link key={s.id} href={`/journal/${s.slug}`} className="group block">
                {s.coverImage && (
