...
export default function SeedFlavoursPage() {
  ...
  const handleSeed = async () => {
    if (!confirm(`Are you sure you want to seed flavours in '${mode}' mode?`)) {
      return;
    }
  ...
  const handleDownloadExample = () => {
    window.location.href = '/api/flavours/seed/download';
  };
  ...
  const handleUpload = async () => {
    ...
    if (!confirm(`Upload and seed flavours from file in '${mode}' mode?`)) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`/api/flavours/seed/upload?mode=${mode}`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to upload flavours');
      }
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
        <h1 className="text-2xl font-semibold mb-2">Seed Flavours</h1>
        <p className="text-gray-600 mb-8">
          Populate the database with initial flavour data from the seed file.
        </p>
            ...
            <div className="space-y-3">
              <label className="flex items-start gap-3 p-4 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-600">
                    Only add flavours that don't already exist. Safe for production.
                  </div>
                </div>
              </label>
              ...
              <label className="flex items-start gap-3 p-4 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-600">
                    Update existing flavours by ID and add new ones. Use with caution.
                  </div>
                </div>
              </label>
              ...
              <label className="flex items-start gap-3 p-4 border border-red-200 rounded-lg cursor-pointer hover:bg-red-50">
                ...
                <div>
                  ...
                  <div className="text-sm text-red-600">
                    Delete all existing flavours and replace with seed data. Cannot be undone!
                  </div>
                </div>
              </label>
            </div>
          ...
          {result && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              ...
              <div className="text-sm text-green-700 space-y-1">
                <div>Mode: {mode}</div>
                <div>Existing flavours: {result.stats.existing}</div>
                <div>New flavours added: {result.stats.seeded}</div>
      ...
      <div className="mt-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
        ...
        <div className="text-sm text-gray-600 space-y-2">
          <p>
            The seed file contains 10 archived flavours from Janine's flavour archive, 
            including classic pairings like Grilled Corn & Wild Tomatoes, Basil Lemon & Raspberry, 
            and Vanilla & Strawberry Thyme.
          </p>
          <p>
            Seed data is located at <code className="px-1 py-0.5 bg-white rounded">lib/seeds/flavours.ts</code>
          </p>
          <p>
            Each flavour includes ingredient relationships, format eligibility, and archive notes.
          </p>
          <p className="text-amber-700 font-medium">
            Note: Make sure to seed ingredients first, as flavours reference ingredient IDs.
          </p>
        </div>
      </div>
    </div>
