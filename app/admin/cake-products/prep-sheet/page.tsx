...
export default function CakePrepSheetPage() {
  ...
  useEffect(() => {
    fetch('/api/orders/upcoming?orderType=cake')
      ...
      .then(setOrders)
      .catch(() => toast.error('Error', 'Failed to load upcoming cake orders'))
  ...
  if (dateGroups.length === 0) return <p className="text-sm text-gray-500 text-center py-16">No upcoming cake orders.</p>;
        ...
        <div key={group.date} className="mb-10">
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-200">
              <h3 className="text-xs font-semibold text-gray-700">Order Details</h3>
            </div>
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {group.orders.map((o) => {
                  const meta = parseCakeMetadata(o.specialInstructions);
                  return (
                    <tr key={o.id}>
                      ...
                      <td className="px-5 py-2 text-xs text-gray-600">{meta.eventType || '—'}</td>
                      <td className="px-5 py-2 text-right font-mono text-gray-900">{meta.numberOfPeople ?? '—'}</td>
                    </tr>
                  );
                })}
              </tbody>
