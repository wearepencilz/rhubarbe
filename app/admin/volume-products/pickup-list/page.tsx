...
interface Order {
  ...
  allergenNotes?: string | null; fulfillmentDate?: string | null;
...
export default function CateringPickupListPage() {
  ...
  if (orders.length === 0) return <p className="text-sm text-gray-500 text-center py-16">No upcoming catering orders.</p>;
                ...
                {dateOrders.map((order) => (
                  <tr key={order.id} className={order.status === 'fulfilled' ? 'bg-green-50/50' : order.status === 'cancelled' ? 'opacity-50' : ''}>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      ...
                      {order.allergenNotes && <span className="block text-xs text-red-600 mt-0.5">{order.allergenNotes}</span>}
                    </td>
