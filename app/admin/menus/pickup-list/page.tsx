...
interface Order {
  ...
  specialInstructions: string | null; status: string;
  launchTitle: string | null; fulfillmentDate: string | null;
  items: OrderItem[];
}
...
export default function MenuPickupListPage() {
  ...
  // Group by menu, then by pickup slot within each menu
          ...
          {menu.slots.map(([slot, slotOrders]) => (
            <div key={slot} className="mb-6">
              <h3 className="text-xs font-medium text-gray-600 mb-3">
                {slot} — {slotOrders.length} order{slotOrders.length !== 1 ? 's' : ''}
              </h3>
