...
export default function MenuOrdersPage() {
  ...
  useEffect(() => {
    fetch('/api/orders?orderType=launch')
      .then((r) => r.json())
      .then(setOrders)
      .catch(() => {})
  ...
  return (
    <TableCard.Root>
      <TableCard.Header
        ...
        description="Orders placed through weekly menus"
        contentTrailing={
      ...
      {loading ? (
        ...
        <div className="py-16 text-center">
          <p className="text-sm text-tertiary">No menu orders found</p>
        </div>
      ) : (
        <Table aria-label="Menu Orders">
          <Table.Header>
          ...
          <Table.Body items={filtered}>
            {(order) => (
              <Table.Row key={order.id} id={order.id} onAction={() => router.push(`/admin/orders/${order.id}`)}>
                ...
                <Table.Cell><span className="text-sm text-secondary">{order.pickupDate ? formatDate(order.pickupDate) : '—'}</span></Table.Cell>
